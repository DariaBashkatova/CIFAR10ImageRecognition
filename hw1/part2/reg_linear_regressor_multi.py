...
class RegularizedLinearRegressor_Multi:
    ...
    def normal_equation(self,X,y,reg):
        ...
        theta_n = (np.dot(np.linalg.inv(np.dot(X.T, X) + reg * np.identity(X.shape[1])), np.dot(X.T, y)))
...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        J_unreg = np.sum([x**2 for x in error]) / (2.0 * num_examples)
        theta_squared = np.sum([x**2 for x in theta[1:]])  # Without first term
        J = J_unreg + (reg * theta_squared) / (2.0 * num_examples)
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return J
    ...
    def grad_loss(self,*args):                                                                          
        ...
        error = np.dot(X, theta) - y
        # theta_altered = np.copy(theta)  # Theta without first term
        # theta_altered[0] = 0.
        # grad = (np.dot(X.T, error) + reg * theta_altered) / (1. * num_examples)
        grad_unreg = (np.dot(X.T, error)) / (1.0 * num_examples)
        grad[0] = grad_unreg[0]
        for j in xrange(1, dim):
            grad[j] = grad_unreg[j] + (reg * theta[j]) / (1. * num_examples)
        # if reg == 0 and reg == 1:
        #     print "GRAD: ", grad
        ###########################################################################
        #                           END OF YOUR CODE                              #
