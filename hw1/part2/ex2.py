...
for reg_param in reg_params:
    ...
    theta_opt = reglinear_reg.train(XX_poly,y,reg=reg_param,num_iters=10000)

    plot_utils.plot_fit(X,y,np.min(X),np.max(X),mu,sigma,theta_opt,p,'Change in water level (x)','Water flowing out of dam (y)','Polynomial Regression fit with lambda = 0 and polynomial features of degree = ' + str(p))
    plt.savefig('FitWithReg=' + str(reg_param) + '.pdf')

    error_train,error_val = utils.learning_curve(XX_poly,y,XX_poly_val,yval,reg_param)
    plot_utils.plot_learning_curve(error_train,error_val,reg_param)
    plt.savefig('LearningCurveWithReg=' + str(reg_param) + '.pdf')
...
reglinear_reg33 = RegularizedLinearReg_SquaredLoss()
reg_param33 = .3
theta_opt33 = reglinear_reg33.train(XX_poly,y,reg=reg_param33,num_iters=10000)
test_error33 = reglinear_reg33.loss(theta_opt33, XX_poly_test, ytest, 0.0)
# test_error33 = np.square(np.dot(XX_poly_test, theta_opt33) - ytest).sum() / (2.0 * ytest.size)

print "TEST_ERROR33: ", test_error33
# error_val[i] = np.square(np.dot(Xtest, theta_opt33) - ytest).sum() / (2.0 * yval.size)

# plot_utils.plot_fit(X,y,np.min(X),np.max(X),mu,sigma,theta_opt33,p,'Change in water level (x)','Water flowing out of dam (y)','Polynomial Regression fit with lambda = 0 and polynomial features of degree = ' + str(p))
# plt.savefig('FitWithReg=' + str(reg_param33) + '.pdf')
#
# error_train,error_val = utils.learning_curve(XX_poly,y,XX_poly_val,yval,reg_param33)
# plot_utils.plot_learning_curve(error_train,error_val,reg_param33)
# plt.savefig('LearningCurveWithReg=' + str(reg_param) + '.pdf')


########################################################################
### Using Regularized Normal Equation to predict on unseen data
reglinear_reg_normal = RegularizedLinearReg_SquaredLoss()
reg_vec, error_train, error_val = utils.validation_curve_normal_equation(XX_poly,y,XX_poly_val,yval)
plot_utils.plot_lambda_selection(reg_vec,error_train,error_val)
plt.savefig('ValidationCurveNormal.pdf')

# reglinear_reg_normal.theta = theta_n
# pred_cost = reglinear_reg_normal.predict(XX.mean(axis=0))
