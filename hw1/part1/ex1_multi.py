...
pred_cost = 10000 * linear_reg3.predict(XX.mean(axis=0))
...
linear_reg4.theta = theta_n
pred_cost = 10000 * linear_reg4.predict(XX1.mean(axis=0))
print 'For average home in Boston suburbs, we predict a median home value of', pred_cost
...
for learning_rate in learning_rates:
    linear_reg3 = LinearReg_SquaredLoss()
    J_history3 = linear_reg3.train(XX,y,learning_rate,num_iters=500,verbose=False)
    plot_utils.plot_data(range(len(J_history3)),J_history3,'Number of iterations','Cost J')
    plt.savefig('LearningRate' + str(learning_rate) + '.pdf')