...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        ##########################################################################
        h = utils.sigmoid(np.dot(X, theta))
        unreg_J = (np.dot(-y, np.log(h)) - np.dot(1 - y, np.log(1 - h))) / m
        J += unreg_J + reg * (np.dot(theta, theta)) / (2.0 * m)
    ...
    def grad_loss(self, *args):
        ...
        ##########################################################################
        h = utils.sigmoid(np.dot(X, theta))
        unreg_grad = (np.dot(X.T, h - y)) / m
        reg_grad_addend = (reg / ((1.0) * m)) * theta
        reg_grad_addend[0] = 0
        grad = unreg_grad + reg_grad_addend
