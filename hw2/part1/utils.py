...
def bin_features(X):
    tX = np.zeros(X.shape)
    # your code here
    tX = np.vectorize(lambda x: 1 if x > 0 else 0)(X)
    # end your code
    return tX
...
def select_lambda_crossval(X,y,lambda_low,lambda_high,lambda_step,penalty):
    ...
    while lambda_test <= lambda_high:
        ...
        for train_index, test_index in kf:
            ...
            if penalty == "l2":
                lreg = linear_model.LogisticRegression(penalty=penalty,C=1.0/lambda_test, solver='lbfgs',fit_intercept=True)  # Orig True
            else:
                lreg = linear_model.LogisticRegression(penalty=penalty,C=1.0/lambda_test, solver='liblinear',fit_intercept=True)  # Orig True
